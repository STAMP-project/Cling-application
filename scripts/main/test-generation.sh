...
for t in ${job_list[@]}; do
  ...
  if [[ "$tool" == evosuite* ]]; then
    ...
    #   target_class=$caller_class
    # fi
    echo "Task #$counter executes Randoop with search_budget $budget seconds for the $execution_id(th) time on project $project. caller class: $caller_class, callee class: $callee_class"
    . scripts/run/run_randoop.sh $execution_id $project $caller_class $callee_class $budget
